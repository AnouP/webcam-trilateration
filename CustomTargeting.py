...
def run():
    ...
    try:
        ...
        angle_height = 42 # 63
        frame_rate = 20
        camera_separation = 40.5 #15.94488 inches
        ...
        targeter1.low_color = np.array([lc_h, lc_s, lc_v])
        targeter1.contour_min_area = 1
        targeter1.targets_max = 1
        targeter1.target_on_contour = False # False = use box size
        targeter1.target_return_box = False # (x,y,bx,by,bw,bh)
        targeter1.target_return_size = False # (x,y,%frame)
        targeter1.contour_draw = False
        ...
        targeter1.high_color = np.array([hc_h, hc_s, hc_v])
        targeter2.contour_min_area = 1
        targeter2.targets_max = 1
        targeter2.target_on_contour = False # False = use box size
        targeter2.target_return_box = False # (x,y,bx,by,bw,bh)
        targeter2.target_return_size = False # (x,y,%frame)
        targeter2.contour_draw = False
            ...
            if (targets1 and targets2):
                ...
                if len(x1k) >= klen:
                    ...
                    if not threadStarted:
                        input("Press anything to start threading:\n")
                        getCoord()
                        threadStarted = True
...
def getCoord():
    threading.Timer(1, getCoord).start ()
    #print(time.perf_counter())
    print(f"x is {X:0.4f} mm")
    row = [X, Y, Z, D]
    writer.writerow(row)
...
class Frame_Motion:
    ...
    # default target color range
    low_color = np.array([18, 50, 84])
    high_color = np.array([28,255,255])
    ...
    # target select
    targets_max = 1 # max targets returned
    ...
    def targets(self,frame):
        ...
        color_mask = cv2.inRange(hsv_frame, self.low_color, self.high_color)
